...
#ifndef __XOKDEPP_OKDE_DIAGONAL_H__
#define __XOKDEPP_OKDE_DIAGONAL_H__
...
namespace xokdepp {
  ...
  class oKDE<SAMPLE_PDF_TYPE, vector_type> : public oKDE_base<SAMPLE_PDF_TYPE, vector_type> {

    typedef gaussian<vector_type> gaussian_type;
    typedef explanation<SAMPLE_PDF_TYPE, vector_type> explanation_type;

  protected:

    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::_dims;

    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::_components;
    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::_weights;
    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::_optimal_bandwidth;

    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::Dth;
    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::_Nt;
    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::Nalpha;
    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::_Mthc;
    using oKDE_base<SAMPLE_PDF_TYPE, vector_type>::_forg;

  public:
    oKDE(size_t dims) :
        oKDE_base<SAMPLE_PDF_TYPE, vector_type>(dims) {
    }

    oKDE(size_t dims, data_type dth) :
        oKDE_base<SAMPLE_PDF_TYPE, vector_type>(dims, dth) {
    }

    // whitening constructor
    oKDE(const oKDE<SAMPLE_PDF_TYPE, vector_type> &to_whiten, const vector_type &smp_mean, const std::vector<int> &eig_ok,
         const std::vector<int> &eig_ko, const matrix_type &F_trns) :
        oKDE_base<SAMPLE_PDF_TYPE, vector_type>(eig_ok.size()) {
      ...
      cov.diagonal() = to_whiten.optimal_bandwidth();
      matrix_type new_full_h = FCFt(cov, F_trns);

      matrix_type new_valid_h(whitened_dims, whitened_dims);
      select_subset_matrix(new_full_h, new_valid_h, eig_ok);
      _optimal_bandwidth = new_valid_h.diagonal();
  ...
  public:
    ...
    data_type compute_rpfg(const mixture<explanation_type> &mix, const vector_type &Fsmpcov, data_type N) const {
      const size_t dims = Fsmpcov.rows();
      matrix_type F = matrix_type::Zero(dims, dims);
      F.diagonal() = Fsmpcov;

      const data_type f = pow(4.0 / ((dims + 2.0) * N), 2.0 / (dims + 4.0));  //THIS IS DIFFERENT FROM PAPER - FOLLOWS KDE CODE IMPL
      const vector_type G = (F * f).diagonal();
      const matrix_type Fsq = F * F; //FIXME?
      ...
      for (size_t i = 0; i < mix.size(); i++) {
        ...
        for (size_t j = i; j < mix.size(); j++) {
          ...
          matrix_type Aij = matrix_type::Zero(dims, dims);
          Aij.diagonal() = ei.covariance_inverse();
    ...
    void estimate_bandwidth() {
      ...
      vector_type F_smp_cov;
      //if(eig_ok.size() > 0){
    	  F_smp_cov = FtCF(smp.covariance(), F);
//      }
//      else{
//    	  vector_type identity_matrix = vector_type::Constant(_dims,1);
//    	  F_smp_cov = FtCF(identity_matrix,F);
//      }
...
//      mixture<explanation_type> n_mix(eig_ok.size(), *this, smp, F);
      ...
      //This should not :)
      data_type F_smp_cov_det = F_smp.covariance_log_determinant() * -0.5;
      data_type innerBandwidthOptScale = exp(F_smp_cov_det - log(N)) / (pow(sqrt(4.0 * M_PI), dims) * dims * Rpfg);

      //just to debug
      if(exp(F_smp.covariance_log_determinant() * -0.5) == std::numeric_limits<data_type>::infinity())
    	  std::cout << "Bandwidth would be Zero in the previous version" << std::endl;
...
//        std::cout << "bandwidthOptScalesquared: " << bandwidthOptScalesquared << std::endl;
//        std::cout << "bandwidthOptScaled: " << bandwidthOptScaled << std::endl;
//        std::cout << "innerBandwidthOptScale: " << innerBandwidthOptScale << std::endl;
//        std::cout << "F_smp_cov: " << std::endl << F_smp_cov << std::endl;
//        std::cout << "F_smp_cov_det: " << F_smp_cov_det << std::endl;
//        std::cout << "F_smp.covariance_log_determinant(): " << F_smp.covariance_log_determinant() << std::endl;
//        std::cout << "F_smp: " << std::endl << F_smp << std::endl;
//        std::cout << "F" << std::endl << F << std::endl;
//        std::cout << "iF" << std::endl << iF << std::endl;
//        std::cout << "moment_matched" << std::endl << moment_matched << std::endl;
//        std::cout << "smp" << std::endl << smp << std::endl;
        _optimal_bandwidth = vector_type::Constant(_dims, xokdepp::MIN_BANDWIDTH);
        return;
        //throw std::exception();
        //std::cout << "" <<  << std::endl;
      }

      _optimal_bandwidth = FtCF(vector_type(H.diagonal()), iF);
      //END V0D00
    }
    ...
    void hierarchical_clustering() {
      ...
      oKDE<SAMPLE_PDF_TYPE, vector_type> whitened_kde(*this, smp.mean(), ev_ok, ev_ko, F_trns);
...
//      std::cout << std::endl;
      ...
      vector_type opt_band = (_optimal_bandwidth / 2) * (0.5 * 0.5);  //to be coherent with what was done with the whitened data
