...
#ifndef __XOKDEPP_EXPLANATION_FULL_H__
#define __XOKDEPP_EXPLANATION_FULL_H__
...
namespace xokdepp {
  ...
  class explanation<SAMPLE_PDF_TYPE, matrix_type> : public explanation_base<SAMPLE_PDF_TYPE, matrix_type> {

    typedef gaussian<matrix_type> gaussian_type;

  protected:

    using explanation_base<SAMPLE_PDF_TYPE, matrix_type>::_dims;
    using explanation_base<SAMPLE_PDF_TYPE, matrix_type>::_mean;
    using explanation_base<SAMPLE_PDF_TYPE, matrix_type>::_covariance;
    using explanation_base<SAMPLE_PDF_TYPE, matrix_type>::_base_covariance;
    using explanation_base<SAMPLE_PDF_TYPE, matrix_type>::underlyingModel;

  public:

    void reset_base_covariance() {
      _base_covariance = matrix_type::Zero(_dims, _dims);
    }

  public:

    inline explanation(size_t dims) :
        explanation_base<SAMPLE_PDF_TYPE, matrix_type>(dims) {
    ...
    inline explanation(const vector_type &mean) :
        explanation_base<SAMPLE_PDF_TYPE, matrix_type>(mean) {
      gaussian_type::reset_covariance();
      reset_base_covariance();
    }
     ...
     * for now we follow the paper and we will use only two gaussians as the underlying model
     */
    inline explanation(const vector_type &mean, const matrix_type &covariance) :
        explanation_base<SAMPLE_PDF_TYPE, matrix_type>(mean, covariance) {
    }

    inline explanation(const gaussian_type &gaussian) :
        explanation<SAMPLE_PDF_TYPE, matrix_type>(gaussian.mean(), gaussian.covariance()) {
    }

    inline explanation(const vector_type &mean, const matrix_type &base_covariance, const mixture<SAMPLE_PDF_TYPE> &underl_model) :
        explanation_base<SAMPLE_PDF_TYPE, matrix_type>(mean, base_covariance, underl_model) {
    }

    //FIXME: should go away!!!
    // merge components
    explanation(const mixture<explanation<SAMPLE_PDF_TYPE, matrix_type>> &mix, const matrix_type &bandwidth) :
        explanation<SAMPLE_PDF_TYPE, matrix_type>(mix.dims()) {
      ...
      //DAVID:FIXME: check gaussian_type
      explanation<SAMPLE_PDF_TYPE, matrix_type> moment_matched = gaussian_type(newDetailedModel);
    ...
    explanation(const mixture<explanation<SAMPLE_PDF_TYPE, matrix_type>> &mix, bool use_base_covariance = true) :
        explanation<SAMPLE_PDF_TYPE, matrix_type>(mix.dims()) {
      ...
      for (size_t i = 0; i < mix.size(); i++) {
        const matrix_type &cov = use_base_covariance ? mix.component(i).base_covariance() : mix.component(i).covariance();
        matrix_type uuT = mix.mean(i) * mix.mean(i).transpose() + cov;
        _base_covariance += mix.weight(i) * uuT;
      }

      _base_covariance -= _mean * _mean.transpose();   //wj^-1 E{wi(Ei+uiuiT)}
      gaussian_type::covariance(_base_covariance);
    }

    // whitening constructor
    // FIXME: check need for dirty bit
    explanation(const explanation_base<SAMPLE_PDF_TYPE, matrix_type> &to_whiten, const vector_type &smp_mean,
                const std::vector<int> &eig_ok, const std::vector<int> &/* eig_ko */, const matrix_type &F_trns) :
        explanation_base<SAMPLE_PDF_TYPE, matrix_type>(eig_ok.size()) {
      ...
      for (size_t dtl_i = 0; dtl_i < to_whiten.detailed_model().size(); dtl_i++) {
        ...
        matrix_type white_cov_full = FCFt(to_whiten.detailed_covariance(dtl_i), F_trns);
        matrix_type white_cov_valid(_dims, _dims);
        select_subset_matrix(white_cov_full, white_cov_valid, eig_ok);

        underlyingModel.add(SAMPLE_PDF_TYPE(white_mean_valid, white_cov_valid), to_whiten.detailed_weight(dtl_i));
      }
